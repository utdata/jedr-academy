...
In our last JedR Trail, we counted the number of characters (rows) that were different species. In this trail we need to use `group_by()` and `summarize()` and `arrange()` again, but we must summarize to get the average height -- or `mean()` -- of each species in the data.

Of note:

- Call the new summarized variable `height_avg` so our AI grade checker knows what to look for.
