...
- If you attempt to match rows containing `NA` values, the result will also be `NA` by default. To address this, you can use `na.rm = TRUE` to exclude rows with NA values.
- Call the new summarized variable `height_avg` so our grade checker knows what to look for.
...
## "In a galaxy filled with towering droids, noble Gungans, resilient Humans, and mighty Wookiees... which species truly stands tallest?"

The tallest species in Star Wars is **Wookiees**.
:::
