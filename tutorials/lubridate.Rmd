---
title: "JedR: Lubridate tutorial"
author: "JedR tutorial series"
tutorial:
...
Hello, young Padawan! In this tutorial we'll look at how the `Lubridate` package can make life easier when dealing with dates in R. In doing so, we'll examine a dataset, scraped from [tvmaze.com](https://www.tvmaze.com/shows/117/star-wars-rebels/episodes), with information about the *Star Wars Rebels* series. Good luck and may the force be with you!
...
rebels_cl <- read_csv("https://github.com/utdata/jedr-training/blob/date-tutorial/data/starwarsrebels.csv?raw=true") |> 
  clean_names() |> 
  mutate(air_date_fixed = dmy(original_air_date)) |> 
  select(-original_air_date)

rebels_cl_newcols <- read_csv("https://github.com/utdata/jedr-training/blob/date-tutorial/data/starwarsrebels.csv?raw=true") |> 
  clean_names() |> 
  mutate(air_date_fixed = dmy(original_air_date)) |> 
...
## Converting ("parsing") dates: Reconizing dates in R 
...
rebels_cl_newcols <- rebels_cl_newcols |> 
  mutate(### USE THE YEAR() FUNCTION HERE TO CREATE A NEW VARIABLE CALLED "YEAR"
           )
...
Notice that the new column, *month_foor*, only include dates that are the first day in any given month? 
...
* In the second line, we're using `geom_bar()` to tell R to make a bar chart. We use the "state = 'identity'" arugment to clarify that we'll be providing the values for the bars
