...
engine: knitr # Not need if teaching python
format: live-html # Format for your Quarto document as a `live` variant
webr:
  packages:
    - tidyverse # Can include any needed packages
    - gradethis 
  render-df: paged-table
live:
  show-hints: true # Can be turned off (false)
  show-solutions: true # Can be turned off (false)
---
...
- Rename `Headline` as `NICAR Trial`

*Question*

- In the `Question` Code Chunk, paste
...
#| label: Question
#| exercise: ex_1
# This is what will show up on the rendered page 
...
In the `Solution` Code Chunk, paste
   starwars |>
   group_by(name) |> 
   summarize (height) |> 
   arrange(desc(height))
...
#| exercise: ex_1
#| solution: true
# This is the solution to the previous code chunk
...
Delete all of these instruction and comments. Then, render!!!